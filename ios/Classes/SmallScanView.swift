...
//        self.buildScanerxx()
    ...
    func initMethodChannel(){
        print("viewId:\(viewId)")
        methodChannel = FlutterMethodChannel.init(name: "plugins.xiaosi.smallscanview_\(viewId)",binaryMessenger: messenger);
...
//     let barcodeView = ScanerView(frame: CGRect(x: UIScreen.main.bounds.size.width * 0.15, y: UIScreen.main.bounds.size.height * 0.15, width: UIScreen.main.bounds.size.width * 0.6, height: UIScreen.main.bounds.size.height * 0.7))
    ...
    var timer: Timer!
    var cameraViewHeight: CGFloat = 176
    ...
    private func initView() {
        ...
        barcodeView.snp.makeConstraints { (make) in
            ...
            make.width.equalTo(cameraView).multipliedBy(0.7)
            make.height.equalTo(cameraView).multipliedBy(0.7)
        }
        //设置扫描线
...
//                  make.right.equalTo(barcodeView).inset(self.scanHeight)
//                  make.bottom.equalTo(barcodeView)
//              }
//              rightView.snp.makeConstraints{ (make) in
//                  make.right.equalTo(cameraView)
//                  make.top.equalTo(barcodeView)
//                  make.left.equalTo(barcodeView).inset(self.scanHeight)
//                  make.bottom.equalTo(barcodeView)
//              }
    }
